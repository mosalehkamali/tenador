...
export async function POST(req, { params }) {
  try {
    ...
    if (payment.status !== 'PENDING' || order.paymentStatus !== 'UNDER_REVIEW') {
      return Response.json({ message: 'Invalid state for rejection' }, { status: 400 });
    }
    ...
    try {
      ...
      payment.meta = { ...payment.meta, rejectedAt: new Date(), rejectedBy: req.user._id };
      ...
      return Response.json({ message: 'Payment rejected' }, { status: 200 });
    } catch (error) {
